...
@hydra.main(config_path='config', config_name='kd', version_base='1.2')
def main(args):
    ...
    for epoch in range(start_epoch, args.epoch):
        ...
        for batch_id, data in tqdm(enumerate(trainDataLoader, 0), total=len(trainDataLoader), smoothing=0.9):
            ...
            # Soft-label KD loss (referred to from docs)
            if args.kd.enabled:
                with torch.no_grad():
                    teacher_pred = teacher(points)
                
                #Soften the student logits by applying softmax first and log() second
                soft_targets = torch.softmax(teacher_pred / t, dim=-1)
                soft_prob = torch.log_softmax(pred / t, dim=-1)

                # Calculate the soft targets loss. Scaled by T**2 as suggested by the authors of the paper "Distilling the knowledge in a neural network"
                soft_targets_loss = torch.sum(soft_targets * (soft_targets.log() - soft_prob)) / soft_prob.size()[0] * (t**2)

                # Weighted sum of the two losses
                loss = alpha * label_loss + (1 - alpha) * soft_targets_loss
            else:
                loss = label_loss
